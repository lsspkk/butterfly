...
import { MapData, ZoneShape, Zone, Point, CoordinateValue, RawZoneShape, MAX_BOUNDARIES } from './MapTypes'
 ...
 */
export function parseCoordinates(tokens: string[], startIndex: number, count: number): CoordinateValue[] {
...
export function parseLevel(tokens: string[], lineNumber: number): number {
  if (tokens.length !== 2) {
    throw new MapParseError(`LEVEL directive expects 1 argument, got ${tokens.length - 1}`, lineNumber)
  }

  const levelNum = parseInt(tokens[1], 10)

  if (isNaN(levelNum) || levelNum < 1) {
    throw new MapParseError(`Invalid level number: ${tokens[1]} (must be positive integer)`, lineNumber)
  }
...
export function parseSize(tokens: string[], lineNumber: number): [number, number] {
  if (tokens.length !== 3) {
    throw new MapParseError(`SIZE directive expects 2 arguments, got ${tokens.length - 1}`, lineNumber)
  }
  ...
  if (isNaN(widthMult) || widthMult <= 0) {
    throw new MapParseError(`Invalid width multiplier: ${tokens[1]} (must be positive number)`, lineNumber)
  }

  if (isNaN(heightMult) || heightMult <= 0) {
    throw new MapParseError(`Invalid height multiplier: ${tokens[2]} (must be positive number)`, lineNumber)
  }
 ...
 */
export function parseShape(tokens: string[], lineNumber: number, directiveName: string): RawZoneShape {
  if (tokens.length < 2) {
    throw new MapParseError(`${directiveName} directive requires shape type`, lineNumber)
  }
  ...
  if (shapeType !== 'rect' && shapeType !== 'ellipse' && shapeType !== 'polygon') {
    throw new MapParseError(`Invalid shape type: ${shapeType} (must be rect, ellipse, or polygon)`, lineNumber)
  }
  ...
  switch (shapeType) {
    ...
    case 'polygon':
      ...
      if (coordCount % 2 !== 0) {
        throw new MapParseError(`Polygon requires even number of coordinates (x,y pairs), got ${coordCount}`, lineNumber)
      }
      // Polygon needs at least 3 vertices (6 coordinates)
      if (coordCount < 6) {
        throw new MapParseError(`Polygon requires at least 3 vertices (6 coordinates), got ${coordCount}`, lineNumber)
      }
  ...
  if (shapeType !== 'polygon' && tokens.length !== expectedCount + 2) {
    throw new MapParseError(`${shapeType} requires ${expectedCount} parameters, got ${tokens.length - 2}`, lineNumber)
  }
...
export function parseBoundary(tokens: string[], lineNumber: number): RawZoneShape {
  if (tokens[0] !== 'BOUNDARY') {
    throw new MapParseError(`Expected BOUNDARY directive, got ${tokens[0]}`, lineNumber)
  }
...
export function parseZone(tokens: string[], lineNumber: number): [string, RawZoneShape] {
  if (tokens[0] !== 'ZONE') {
    throw new MapParseError(`Expected ZONE directive, got ${tokens[0]}`, lineNumber)
  }

  if (tokens.length < 2) {
    throw new MapParseError('ZONE directive requires zone ID', lineNumber)
  }
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(zoneId)) {
    throw new MapParseError(`Invalid zone ID: ${zoneId} (must be alphanumeric with _ or -)`, lineNumber)
  }
...
export function parseCatSpawn(tokens: string[], lineNumber: number): [CoordinateValue, CoordinateValue] {
  if (tokens[0] !== 'CAT_SPAWN') {
    throw new MapParseError(`Expected CAT_SPAWN directive, got ${tokens[0]}`, lineNumber)
  }

  if (tokens.length !== 3) {
    throw new MapParseError(`CAT_SPAWN directive expects 2 arguments, got ${tokens.length - 1}`, lineNumber)
  }
 ...
 */
export function convertRawShapeToShape(rawShape: RawZoneShape, worldWidth: number, worldHeight: number): ZoneShape {
 ...
 */
export function parseMapFile(content: string, viewportWidth: number, viewportHeight: number): MapData[] {
  ...
  for (const { tokens, lineNumber } of tokenizedLines) {
    ...
    try {
      if (directive === 'LEVEL') {
        ...
        if (currentLevel.boundaries.length >= MAX_BOUNDARIES) {
          throw new MapParseError(`Maximum ${MAX_BOUNDARIES} BOUNDARY directives allowed per level`, lineNumber)
        }
        currentLevel.boundaries.push(parseBoundary(tokenValues, lineNumber))
 ...
 */
function finalizeLevelBuilder(builder: LevelBuilder, viewportWidth: number, viewportHeight: number): MapData {
  ...
  if (builder.boundaries.length === 0) {
  ...
  const boundaries = builder.boundaries.map((raw) => convertRawShapeToShape(raw, worldWidth, worldHeight))
