...
describe('MapParser - Directives', () => {
  ...
  describe('parseBoundary', () => {
    describe('rect shapes', () => {
      ...
      it('should throw on rect with too few parameters', () => {
        expect(() => parseBoundary(['BOUNDARY', 'rect', '10%', '10%', '80%'], 1))
          .toThrow('rect requires 4 parameters, got 3');
      });

      it('should throw on rect with too many parameters', () => {
        expect(() => parseBoundary(['BOUNDARY', 'rect', '10%', '10%', '80%', '80%', '50%'], 1))
          .toThrow('rect requires 4 parameters, got 5');
      });
    });
    ...
    describe('ellipse shapes', () => {
      ...
      it('should throw on ellipse with too few parameters', () => {
        expect(() => parseBoundary(['BOUNDARY', 'ellipse', '50%', '50%', '40%'], 1))
          .toThrow('ellipse requires 4 parameters, got 3');
      });

      it('should throw on ellipse with too many parameters', () => {
        expect(() => parseBoundary(['BOUNDARY', 'ellipse', '50%', '50%', '40%', '40%', '30%'], 1))
          .toThrow('ellipse requires 4 parameters, got 5');
      });
    });
    ...
    describe('polygon shapes', () => {
      ...
      it('should throw on polygon with too few vertices', () => {
        expect(() => parseBoundary(['BOUNDARY', 'polygon', '50%', '10%', '90%', '90%'], 1))
          .toThrow('Polygon requires at least 3 vertices (6 coordinates), got 4');
      });

      it('should throw on polygon with odd number of coordinates', () => {
        expect(() => parseBoundary(['BOUNDARY', 'polygon', '50%', '10%', '90%', '90%', '10%'], 1))
          .toThrow('Polygon requires even number of coordinates (x,y pairs), got 5');
      });
    });

    describe('error handling', () => {
      it('should throw on missing shape type', () => {
        expect(() => parseBoundary(['BOUNDARY'], 1))
          .toThrow('BOUNDARY directive requires shape type');
      });

      it('should throw on invalid shape type', () => {
        expect(() => parseBoundary(['BOUNDARY', 'circle', '50%', '50%', '40%'], 1))
          .toThrow('Invalid shape type: circle');
      });

      it('should throw on wrong directive name', () => {
        expect(() => parseBoundary(['ZONE', 'rect', '10%', '10%', '80%', '80%'], 1))
          .toThrow('Expected BOUNDARY directive, got ZONE');
      });

      it('should throw on invalid coordinate value', () => {
        expect(() => parseBoundary(['BOUNDARY', 'rect', 'abc', '10%', '80%', '80%'], 1))
          .toThrow('Invalid numeric value');
      });

      it('should throw on percentage out of range', () => {
        expect(() => parseBoundary(['BOUNDARY', 'rect', '150%', '10%', '80%', '80%'], 1))
          .toThrow('must be between 0 and 100');
      });

      it('should throw on negative percentage', () => {
        expect(() => parseBoundary(['BOUNDARY', 'rect', '-10%', '10%', '80%', '80%'], 1))
          .toThrow('must be between 0 and 100');
      });

      it('should throw on negative absolute coordinate', () => {
        expect(() => parseBoundary(['BOUNDARY', 'rect', '-50', '50', '400', '300'], 1))
  ...
  describe('parseShape', () => {
    ...
    it('should include directive name in error messages', () => {
      expect(() => parseShape(['ZONE'], 1, 'ZONE'))
        .toThrow('ZONE directive requires shape type');
    });
  });
  ...
  describe('parseZone', () => {
    describe('valid zones', () => {
      ...
      it('should parse zone with alphanumeric ID', () => {
        const [id, shape] = parseZone(['ZONE', 'zone1', 'rect', '10%', '10%', '20%', '20%'], 1);
        expect(id).toBe('zone1');
      });

      it('should parse zone with underscore in ID', () => {
        const [id, shape] = parseZone(['ZONE', 'bottom_left', 'rect', '10%', '10%', '20%', '20%'], 1);
        expect(id).toBe('bottom_left');
      });

      it('should parse zone with hyphen in ID', () => {
        const [id, shape] = parseZone(['ZONE', 'top-right', 'rect', '10%', '10%', '20%', '20%'], 1);
    ...
    describe('error handling', () => {
      it('should throw on missing zone ID', () => {
        expect(() => parseZone(['ZONE'], 1))
          .toThrow('ZONE directive requires zone ID');
      });

      it('should throw on invalid zone ID with spaces', () => {
        expect(() => parseZone(['ZONE', 'my zone', 'rect', '10%', '10%', '20%', '20%'], 1))
          .toThrow('Invalid zone ID: my zone');
      });

      it('should throw on invalid zone ID with special chars', () => {
        expect(() => parseZone(['ZONE', 'zone@1', 'rect', '10%', '10%', '20%', '20%'], 1))
          .toThrow('Invalid zone ID: zone@1');
      });

      it('should throw on empty zone ID', () => {
        expect(() => parseZone(['ZONE', '', 'rect', '10%', '10%', '20%', '20%'], 1))
          .toThrow('Invalid zone ID: ');
      });

      it('should throw on wrong directive name', () => {
        expect(() => parseZone(['BOUNDARY', 'center', 'rect', '10%', '10%', '20%', '20%'], 1))
          .toThrow('Expected ZONE directive, got BOUNDARY');
      });

      it('should throw on missing shape type', () => {
        expect(() => parseZone(['ZONE', 'center'], 1))
          .toThrow('ZONE directive requires shape type');
      });

      it('should throw on invalid shape type', () => {
        expect(() => parseZone(['ZONE', 'center', 'circle', '50%', '50%', '40%'], 1))
          .toThrow('Invalid shape type: circle');
      });

      it('should throw on rect with too few parameters', () => {
        expect(() => parseZone(['ZONE', 'center', 'rect', '10%', '10%', '20%'], 1))
          .toThrow('rect requires 4 parameters, got 3');
      });

      it('should throw on ellipse with too many parameters', () => {
        expect(() => parseZone(['ZONE', 'center', 'ellipse', '50%', '50%', '40%', '40%', '30%'], 1))
          .toThrow('ellipse requires 4 parameters, got 5');
      });

      it('should throw on polygon with too few vertices', () => {
        expect(() => parseZone(['ZONE', 'tri', 'polygon', '50%', '10%', '90%', '90%'], 1))
          .toThrow('Polygon requires at least 3 vertices (6 coordinates), got 4');
      });

      it('should throw on polygon with odd number of coordinates', () => {
        expect(() => parseZone(['ZONE', 'tri', 'polygon', '50%', '10%', '90%', '90%', '10%'], 1))
          .toThrow('Polygon requires even number of coordinates (x,y pairs), got 5');
      });

      it('should throw on invalid coordinate value', () => {
        expect(() => parseZone(['ZONE', 'center', 'rect', 'abc', '10%', '20%', '20%'], 1))
          .toThrow('Invalid numeric value');
      });

      it('should throw on percentage out of range', () => {
        expect(() => parseZone(['ZONE', 'center', 'rect', '150%', '10%', '20%', '20%'], 1))
          .toThrow('must be between 0 and 100');
      });

      it('should throw on negative absolute coordinate', () => {
        expect(() => parseZone(['ZONE', 'center', 'rect', '-50', '50', '200', '150'], 1))
          .toThrow('must be non-negative');
      });
    });
  });
  ...
  describe('parseCatSpawn', () => {
    ...
    describe('error handling', () => {
      it('should throw on missing arguments', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN'], 1))
          .toThrow('expects 2 arguments, got 0');
      });

      it('should throw on one argument only', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '50%'], 1))
          .toThrow('expects 2 arguments, got 1');
      });

      it('should throw on too many arguments', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '50%', '80%', '100%'], 1))
          .toThrow('expects 2 arguments, got 3');
      });

      it('should throw on wrong directive name', () => {
        expect(() => parseCatSpawn(['ZONE', '50%', '80%'], 1))
          .toThrow('Expected CAT_SPAWN directive, got ZONE');
      });

      it('should throw on invalid x coordinate', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', 'abc', '80%'], 1))
          .toThrow('Invalid numeric value');
      });

      it('should throw on invalid y coordinate', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '50%', 'xyz'], 1))
          .toThrow('Invalid numeric value');
      });

      it('should throw on percentage out of range for x', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '150%', '80%'], 1))
          .toThrow('must be between 0 and 100');
      });

      it('should throw on percentage out of range for y', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '50%', '-10%'], 1))
          .toThrow('must be between 0 and 100');
      });

      it('should throw on negative absolute x', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '-50', '300'], 1))
          .toThrow('must be non-negative');
      });

      it('should throw on negative absolute y', () => {
        expect(() => parseCatSpawn(['CAT_SPAWN', '400', '-100'], 1))
          .toThrow('must be non-negative');
      });
    });
  });
  ...
  describe('convertRawShapeToShape', () => {
    ...
    describe('error handling', () => {
      it('should throw on rect with wrong parameter count', () => {
        ...
        expect(() => convertRawShapeToShape(rawShape, worldWidth, worldHeight))
          .toThrow('rect requires 4 parameters');
      });
      ...
      it('should throw on ellipse with wrong parameter count', () => {
        ...
        expect(() => convertRawShapeToShape(rawShape, worldWidth, worldHeight))
          .toThrow('ellipse requires 4 parameters');
      });
      ...
      it('should throw on polygon with too few vertices', () => {
        ...
        expect(() => convertRawShapeToShape(rawShape, worldWidth, worldHeight))
          .toThrow('polygon requires at least 6 parameters');
      });
...
CAT_SPAWN 50% 80%
      ...
      expect(map.boundary.type).toBe('rect');
...
CAT_SPAWN 50% 50%
      ...
      const map = maps[0];
      expect(map.boundary.type).toBe('polygon');
      if (map.boundary.type === 'polygon') {
        expect(map.boundary.points).toHaveLength(3);
      }
    });
...
CAT_SPAWN 50% 50%
      ...
      if (map.boundary.type === 'rect') {
        expect(map.boundary.x).toBe(worldWidth * 0.1);
        expect(map.boundary.y).toBe(worldHeight * 0.1);
        expect(map.boundary.width).toBe(worldWidth * 0.8);
        expect(map.boundary.height).toBe(worldHeight * 0.8);
      }
...
CAT_SPAWN 50% 80%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('missing SIZE directive');
      });
...
CAT_SPAWN 50% 80%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('missing BOUNDARY directive');
      });
...
ZONE center rect 30% 30% 40% 40%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('missing CAT_SPAWN directive');
      });
...
CAT_SPAWN 50% 80%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('must have at least one ZONE');
      });

      it('should throw on duplicate BOUNDARY directive', () => {
        const content = `
LEVEL 1
SIZE 1.0 1.0
BOUNDARY rect 0 0 100% 100%
...
CAT_SPAWN 50% 80%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('Duplicate BOUNDARY directive');
      });
...
CAT_SPAWN 60% 70%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('Duplicate CAT_SPAWN directive');
      });
...
CAT_SPAWN 50% 80%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('Duplicate zone ID: center');
      });
...
CAT_SPAWN 50% 80%
        ...
        expect(() => parseMapFile(content, viewportWidth, viewportHeight))
          .toThrow('Unknown directive: UNKNOWN_DIRECTIVE');
      });
    });
  });
});
