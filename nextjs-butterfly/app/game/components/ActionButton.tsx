...
import { keyMap } from '../systems/KeyboardListener'
...
export function ActionButton({
  ...
  return (
    <button onClick={onClickWithInterval} className={`opacity-70 ${zIndex} ${location}`}>
      <svg className='w-[20vh] h-[20vh]' xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 28 28' stroke='currentColor'>
        ...
        <circle cx='14' cy='14' r='10' strokeWidth={2} fill='none' className='opacity-25 shadow-sm stroke-black shadow-black' />
        <circle cx='14' cy='14' r='10' strokeWidth={1} fill='none' className='shadow-sm stroke-gray-700 shadow-black' />
        {allowAction && (
          <>
            <circle cx='13' cy='13' r='11' strokeWidth={0} className={`stroke-sky-800 fill-sky-400 ${className ?? ''}`} />
            <circle cx='13' cy='13' r='8' strokeWidth={1} className={`stroke-sky-400 fill-blue-300 shadow-blue-100 shadow-sm ${className ?? ''}`} />
          </>
        )}
        {!allowAction && (
          <>
            <circle cx='13' cy='13' r='11' strokeWidth={0} className={`stroke-sky-900 fill-sky-700 ${className ?? ''}`} />
            <circle cx='13' cy='13' r='8' strokeWidth={1} className={`stroke-sky-600 fill-blue-500 shadow-sm ${className ?? ''}`} />
          </>
        )}
      </svg>
    </button>
